...
if ($endpoint === 'list-messages' && $method === 'GET') {
        ...
        case 'send-bulk-sms':
            if ($method === 'POST') {
                ...
                foreach ($recipients as $recipient) {
                    ...
                    try {
                        $apiResult = $smsAPI->sendSingleSMS($mobile, $message);
                        $success = is_array($apiResult)
                            ? (($apiResult['success'] ?? false) || (($apiResult['response-code'] ?? $apiResult['respose-code'] ?? null) == 200))
                            : (bool) $apiResult;
