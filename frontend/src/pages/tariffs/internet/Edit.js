...
const Add = ({ ...props }) => {
    ...
    const formik = useFormik({
        initialValues: {
            router: { id: data?.router_id ?? '', title: data?.router_name ?? "" },
            title: data?.title ?? '',
            price: data?.price ?? '',
            weekly_price: data?.weekly_price ?? '',
            bi_weekly_price: data?.bi_weekly_price ?? '',
            rate_limit: { value: data?.rate_limit?.value ?? '', label: data?.rate_limit?.label ?? "" },
        },
        ...
        const fetchPlan = async (id) => {
            ...
            try {
                if (id !== undefined || null || "") {
                    const response = await http.get(`${process.env.REACT_APP_API_URL}/view-plans/${id}`);
                    if (response.data?.plan) {
                        setData(response.data?.plan);
                        setValue(response.data?.plan?.router_id);
                    }
                }
                setApiLoading(false);
            } catch (error) {
                                ...
                                {apiLoading ?
                                    ...
                                    <form className="gy-3 is-alter custom-form" onSubmit={formik.handleSubmit}>
                                        ...
                                        <Row className="align-center">
                                            <Col md="4" className="col-form-label">
                                                <div className="form-group">
                                                    <label className="form-label" htmlFor="weekly_price">
                                                        Weekly
                                                    </label>
                                                </div>
                                            </Col>
                                                    ...
                                                    <div className="form-control-wrap">
                                                        <input
                                                            ...
                                                            className={classnames(
                                                                'form-control',
                                                                {
                                                                    'is-invalid': formik.touched.weekly_price && formik.errors.weekly_price
                                                                }
                                                            )}
                                                            name="weekly_price"
                                                            value={formik.values.weekly_price}
                                                            onChange={formik.handleChange}
                                                        />
                                                        {formik.touched.weekly_price && formik.errors.weekly_price ? (<p className="invalid">{formik.errors.weekly_price}</p>) : null}
                                                    </div>
                                                </div>
                                            </Col>
                                        </Row>
                                        <Row className="align-center">
                                            <Col md="4" className="col-form-label">
                                                <div className="form-group">
                                                    <label className="form-label" htmlFor="bi_weekly_price">
                                                        Bi-Weekly
                                                    </label>
                                                </div>
                                            </Col>
                                                    ...
                                                    <div className="form-control-wrap">
                                                        <input
                                                            ...
                                                            className={classnames(
                                                                'form-control',
                                                                {
                                                                    'is-invalid': formik.touched.bi_weekly_price && formik.errors.bi_weekly_price
                                                                }
                                                            )}
                                                            name="bi_weekly_price"
                                                            value={formik.values.bi_weekly_price}
                                                            onChange={formik.handleChange}
                                                        />
                                                        {formik.touched.bi_weekly_price && formik.errors.bi_weekly_price ? (<p className="invalid">{formik.errors.bi_weekly_price}</p>) : null}
                                                    </div>
                                                </div>
                                            </Col>
                                        </Row>
