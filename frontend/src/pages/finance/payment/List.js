...
const List = ({ ...props }) => {
    ...
    useEffect(() => {
        const fetchPayments = async () => {
            setLoading(true);
            const response = await http.get(`${process.env.REACT_APP_API_URL}/list-payments?q=${searchText}&page=${page}&per_page=${perPage}&sort_col=${sortCol}&sort=${sort}&start=${startDate}&end=${endDate}`);
