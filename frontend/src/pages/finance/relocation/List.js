...
const RelocationList = () => {
  ...
  const [amount, setAmount] = useState(String(RELOCATION_MIN_AMOUNT));
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setRows(mergeRelocationRows(tickets, txns));
    } catch (err) {
      showError(err?.response?.data?.message || err.message || "Failed to load relocations");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const openCollect = (row) => {
    setActive(row);
    setAmount(String(Math.max(RELOCATION_MIN_AMOUNT, Number(row.amount) || RELOCATION_MIN_AMOUNT)));
  ...
  const sendStk = async () => {
    ...
    if (!Number.isFinite(amt) || amt < RELOCATION_MIN_AMOUNT) {
      showError(`Amount must be at least ${RELOCATION_MIN_AMOUNT}`);
      return;
    }
    if (!phone.trim()) {
  ...
  return (
    <React.Fragment>
      <Head title="Relocation" />
      <Content>
        <div className="reloc-finance">
          <BlockHead size="sm">
            <BlockBetween>
              <BlockHeadContent>
                <BlockTitle page>Relocation</BlockTitle>
              </BlockHeadContent>
              <BlockHeadContent>
                <Button
                  tag={Link}
                  to={`${process.env.PUBLIC_URL}/admin/finance/relocation/dashboard`}
                  color="light"
                  size="sm"
                >
                  Dashboard
            ...
            <PreviewCard>
              ...
              {loading ? (
                <div className="text-center py-5">
                  <Spinner size="sm" /> Loading relocation tickets…
                </div>
              ) : filtered.length === 0 ? (
                <p className="text-soft mb-0 py-4 text-center">No relocation tickets match.</p>
              ) : (
                        ...
                        <tr key={t.id || num}>
                          ...
                          <td className="text-nowrap">
                            {row.status === "paid"
                              ...
                              : formatKes(RELOCATION_MIN_AMOUNT)}
                            {row.transaction?.receipt ? (
        ...
        <Modal isOpen={collectOpen} toggle={closeCollect} className="modal-dialog-centered reloc-finance" size="md">
          <ModalHeader toggle={closeCollect}>Collect relocation fee</ModalHeader>
          <ModalBody>
            {active?.ticket ? (
            ...
            <FormGroup>
              ...
              <Input
                type="number"
                min={RELOCATION_MIN_AMOUNT}
                step="1"
                value={amount}
                onChange={(e) => setAmount(e.target.value)}
              />
              <small className="text-soft">Minimum {RELOCATION_MIN_AMOUNT}. You may increase.</small>
            </FormGroup>
            <FormGroup>
