...
const Inbox = () => {
    ...
    const columns = [
        ...
        },
        {
            ...
            name: "From",
            cell: (row) => (
                row.customer_id ? <Link to={`${process.env.PUBLIC_URL}/admin/customers/view/${row.customer_id}`}>{row.name}</Link> : row.from
            ),
        ...
        },
        {
            id: 3,
            name: "Message",
            cell: (row) => (<p className="py-3">
                {row.message_description?.body ? row.message_description.body.length > 80 ? row.message_description.body.substr(0, 80) + '...' : row.message_description.body : row.message_description?.emoji ? nameToEmoji[row.message_description.emoji] :
                    row.message_description?.image ? (<>
                        <Link to={row.message_description.image} download><img src={row.message_description.image} width="100" /></Link>
                        {row.message_description?.caption ? <><br />{row.message_description?.caption} </> : ''}
                    </>) :
                        row.message_description?.video ? (<>
                            <video width="100" controls>
                                <source type="video/mp4" src={row.message_description.video} />
                            </video>
                            {row.message_description?.caption ? <><br />{row.message_description?.caption} </> : ''}
                        </>)
                            : row.message_description
                }
                {row.reply_id ? <><br />Replied to: <Link to={`${process.env.PUBLIC_URL}/admin/whatsapp/outbox/${row.reply_id}`}>{row.reply_id}</Link></> : ''}
            </p>),
            selector: (row) => row.message_description,
            grow: 3,
            wrap: true,
            sortable: false,
            ref: "message_description"
        },
        {
            id: 4,
            name: "Status",
            cell: (row) =>
                row.status === 'sent' ? <Badge color="success">Received</Badge> : <Badge color="light">Unkwown</Badge>
            ,
    ...
    useEffect(() => {
        const fetchMessages = async () => {
            setLoading(true);
            const response = await http.get(`${process.env.REACT_APP_API_URL}/list-inbox-whatsapp?q=${searchText}&page=${page}&per_page=${perPage}&sort_col=${sortCol}&sort=${sort}`);
    ...
    return (
        <>
            <Head title="Incoming whatsapp messages" />
            <Content>
