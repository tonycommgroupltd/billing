...
import { Block, BlockHead, BlockHeadContent, BlockTitle, RSelect, Icon, Col, Row, PreviewCard } from "../../components/Component";
...
const CreateTicket = () => {
  ...
  useEffect(() => {
    ...
    if (!customerPhone && !customerName && !customerId) return;

    const normalizedPhone = normalizeKenyanPhone(customerPhone) || customerPhone;
    setFormData((prev) => ({
      ...prev,
    ...
    if (normalizedPhone && normalizedPhone.length >= 10) {
      fetchCustomerTickets(normalizedPhone);
    }
  }, [searchParams]);
