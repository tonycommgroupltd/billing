...
const List = ({ user }) => {
    ...
    const columns = [
        ...
        },
        {
            id: 4,
            name: "Role",
            selector: (row) => row.role_display_name,
            wrap: true,
            sortable: false,
            ref: "role_display_name"
        },
        {
            name: "Actions",
            cell: (row) => (
    ...
    useEffect(() => {
        const fetchAdministrators = async () => {
            setLoading(true);
            const response = await http.get(`${process.env.REACT_APP_API_URL}/list-administrators?q=${searchText}&page=${page}&per_page=${perPage}&sort_col=${sortCol}&sort=${sort}`);
    ...
    const formik = useFormik({
        ...
        validationSchema: Yup.object({
            ...
            phone: Yup.string()
                .matches(
                    phoneRegex,
                    "Invalid characters in phone number field (allowed only 0-9)"
                )
                .nullable(),
    ...
    const formikEdit = useFormik({
        initialValues: {
            ...
            role: { value: formData?.role_name ?? '', label: formData?.role_display_name ?? "None" },
        },
        ...
        onSubmit: (data, { resetForm }) => {
            setFormLoading(true);
            http
                .post(`/update-administrators/${data.id}`, {
                    name: data.name,
                    email: data.email,
                    phone: data.phone,
                    role: data.role.value
        ...
        const fetchAdministrator = async (id) => {
            ...
            try {
                if (id !== undefined || null || "") {
                    const response = await http.get(`${process.env.REACT_APP_API_URL}/view-administrators/${id}`);
                    ...
                    <form className="gy-3 is-alter custom-form" onSubmit={formik.handleSubmit}>
                        ...
                        <Row className="align-center">
                            ...
                            <Col md="8">
                                <div className="form-group">
                                    <div className="form-control-wrap">
                                        <RSelect name="role" options={rolesSchemeOptions} value={formik.values.role}
                        ...
                        <form className="gy-3 is-alter custom-form" onSubmit={formikEdit.handleSubmit}>
                            ...
                            <Row className="align-center">
                                ...
                                <Col md="8">
                                    <div className="form-group">
                                        <div className="form-control-wrap">
                                            <RSelect name="role" options={rolesSchemeOptions} value={formikEdit.values.role}
