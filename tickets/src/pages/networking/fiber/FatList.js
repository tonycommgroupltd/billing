...
const FatList = () => {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const response = await fetch(`${apiUrl}/network-map.php?type=fat`);
      
      if (!response.ok) throw new Error('Failed to fetch FAT data');
      
      const result = await response.json();
      if (result.success) {
        setData(result.fat || []);
        // Extract unique hub names
        const uniqueHubs = [...new Set((result.fat || []).map(f => f.hub_name).filter(Boolean))];
        setHubs(uniqueHubs);
      } else {
        throw new Error(result.error || 'Failed to load data');
      }
    } catch (err) {
      console.error('Error loading FAT data:', err);
  ...
  // Start editing a FAT's splitter counts
  ...
  return (
    <>
      <Head title="FAT Points - Fiber Structure" />
      <Content>
        <BlockHead size="sm">
          <BlockBetween>
            <BlockHeadContent>
              ...
              <p className="text-soft">Fiber Access Terminals in the network</p>
            </BlockHeadContent>
            <BlockHeadContent>
            ...
            <div className="card-inner">
              ...
              {loading && (
                <div className="text-center py-5">
                  ...
                  <p className="mt-2">Loading FAT points...</p>
                </div>
              )}
              ...
              {!loading && !error && (
                <div style={{overflowX: 'auto', WebkitOverflowScrolling: 'touch'}}>
                  ...
                  {filteredData.length === 0 ? (
                    <div className="text-center py-4 text-soft">No FAT points found</div>
                  ) : (
                    filteredData.map((item, index) => {
                      ...
                      return (
                        <DataTableItem key={item.id}>
                          ...
                          <DataTableRow>
                            <span className="fw-bold text-primary">{item.fid || `FAT-${item.id}`}</span>
                          </DataTableRow>
                          ...
                          <DataTableRow className="text-center">
                            <Badge color={totalSplitters > 0 ? 'primary' : 'secondary'}>
                              {totalSplitters}
                            </Badge>
                          </DataTableRow>
