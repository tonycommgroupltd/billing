...
import CustomerDateTimeCell, { customerDateTimeSelector } from "../../components/customer/CustomerDateTimeCell";
...
const List = ({ ...props }) => {
    ...
    const columns = [
        ...
        },
        {
            ...
            name: "Created",
            cell: (row) => (
                <CustomerDateTimeCell value={row.ticket_created_at || row.created_at} />
            ),
            selector: (row) =>
                customerDateTimeSelector(row, row.ticket_created_at ? "ticket_created_at" : "created_at"),
            wrap: true,
            sortable: true,
            ref: "created_at",
            minWidth: "140px",
        },
        {
            ...
            name: "Updated",
            cell: (row) => (
                <CustomerDateTimeCell value={row.ticket_updated_at || row.updated_at} />
            ),
            selector: (row) =>
                customerDateTimeSelector(row, row.ticket_updated_at ? "ticket_updated_at" : "updated_at"),
            wrap: true,
            sortable: true,
            ref: "updated_at",
            minWidth: "140px",
        },
        {
            ...
            cell: (row) => (
                <>
                    {row.services.map((tag, i) => {
                        return <div key={`ttl${tag.id}`}>
                            <div className="button-link">
                                {tag.plan_title}
                            </div> {i < row.services.length - 1 ? ', ' : null}
                        </div>
                    })}
                </>
            ),
    ...
    useEffect(() => {
        const fetchCustomers = async () => {
            setLoading(true);
            const response = await http.get(`${process.env.REACT_APP_API_URL}/list-customers?q=${searchText}&page=${page}&per_page=${perPage}&sort_col=${sortCol}&sort=${sort}&status=${status}`);
