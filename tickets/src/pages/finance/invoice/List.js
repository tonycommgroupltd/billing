...
const List = ({ ...props }) => {
    ...
    useEffect(() => {
        const fetchInvoices = async () => {
            setLoading(true);
            const response = await httpNode.get(`/finance/invoices?q=${searchText}&page=${page}&per_page=${perPage}&sort_col=${sortCol}&sort=${sort}&start=${startDate}&end=${endDate}&status=${invoiceStatus}`);
        ...
        const fetchInvoice = async (id) => {
            ...
            try {
                if (id !== undefined || null || "") {
                    const response = await httpNode.get(`/finance/invoices/${id}`);
