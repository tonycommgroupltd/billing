...
const TicketsDashboard = ({ user }) => {
  ...
  const canFetchAdministrators = () => {
    const roles = (user?.all_roles || []).map((r) => String(r).toLowerCase());
    return roles.some((role) =>
      ['administrator', 'super-administrator', 'super-admin', 'manager'].includes(role)
    );
  };

  // Fetch administrators from the database (admins/managers only)
  const fetchAdministrators = async () => {
    if (!canFetchAdministrators()) {
      return [];
    }
    try {
