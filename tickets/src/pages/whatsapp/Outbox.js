...
const Outbox = () => {
    ...
    const columns = [
        {
            ...
            name: "ID",
            cell: (row) => (
                <Link to={`${process.env.PUBLIC_URL}/admin/whatsapp/outbox/${row.id}`}>{row.id}</Link>
            ),
            selector: (row) => row.id,
            width: "80px",
            wrap: true,
        ...
        },
        {
            id: 3,
            name: "Message",
            cell: (row) => row.message_description && row.message_description.length > 80 ? row.message_description.substr(0, 80) + '...' : row.message_description,
        ...
        },
        {
            ...
            name: "Status",
            cell: (row) =>
                row.status === 'sent' ? <Badge color="primary">Sent</Badge> : row.status === 'delivered' ? <Badge color="success">Delivered</Badge> : row.status === 'deleted' ? <Badge color="danger">Deleted</Badge> : row.status === 'failed' ? <Badge color="danger">Failed</Badge> : row.status === 'queued' ? <Badge color="light">Queued</Badge> :  row.status === 'read' ? <Badge color="success">Read</Badge> : row.status === 'deleted' ? <Badge color="danger">Deleted</Badge> : <Badge color="light">Unknown</Badge>
            ,
        ...
        },
        {
            ...
            cell: (row) => (
                <ul className="nk-tb-actions gx-1">
                    <li>
                        <Link to={`${process.env.PUBLIC_URL}/admin/whatsapp/outbox/${row.id}`}>
                            <Open20Regular />
                        </Link>
                    </li>
                    <li>
    ...
    useEffect(() => {
        const fetchMessages = async () => {
            setLoading(true);
            const response = await http.get(`${process.env.REACT_APP_API_URL}/list-outbox-whatsapp?q=${searchText}&page=${page}&per_page=${perPage}&sort_col=${sortCol}&sort=${sort}`);
    ...
    return (
        <>
            <Head title="Outgoing whatsapp messages" />
            <Content>
