...
const Outbox = () => {
    ...
    const columns = [
        {
            ...
            name: "ID",
            cell: (row) => (
                <Link to={`/admin/sms/outbox/${row.id}`}>{row.id}</Link>
            ),
            selector: (row) => row.id,
            width: "80px",
            wrap: true,
        ...
        },
        {
            ...
            name: "Customer",
            cell: (row) => (
                row.customer_id ? <Link to={`/admin/customers/view/${row.customer_id}`}>{row.name}</Link> : row.recipient
            ),
        ...
        },
        {
            ...
            cell: (row) => (
                <ul className="nk-tb-actions gx-1">
                    ...
                    <li>
                        <Link to={`/admin/sms/outbox/${row.id}`}>
                            <Open20Regular />
                        </Link>
                    </li>
                    <li>
    ...
    useEffect(() => {
        const fetchMessages = async () => {
            setLoading(true);
            try {
                const response = await http.get(`/list-messages?q=${encodeURIComponent(searchText)}&page=${page}&per_page=${perPage}&sort_col=${sortCol}&sort=${sort}`);
                setData(response.data?.data || []);
                setTotalRows(response.data?.total ?? 0);
            } catch (err) {
                console.error('Load outbox error:', err);
                setData([]);
                setTotalRows(0);
                showError(err.response?.data?.error || 'Failed to load SMS outbox from Splynx database');
            } finally {
                setLoading(false);
            }
        };
            ...
            <Content>
                <BlockHead size="lg" wide="sm">
                    <BlockHeadContent>
                        ...
                        <p className="text-soft" style={{ fontSize: '13px', marginTop: '6px' }}>
                            Loaded from Splynx database (message_details on server)
                        </p>
                    </BlockHeadContent>
                </BlockHead>
