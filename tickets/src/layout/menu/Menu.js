...
import menuManager from "./MenuManagerData";
import menuCust from "./MenuCustData";
import menuCustomerCreator from "./MenuCustomerCreatorData";
import menuTechnician from "./MenuTechnicianData";
import menuEngineer from "./MenuEngineerData";
...
const MenuItem = ({ icon, link, text, sub, newTab, sidebarToggle, mobileView, badge, ...props }) => {
  ...
  const getSubMenuHeight = (subMenuEl, subMenuItems) => {
    if (mobileView && subMenuEl) {
      return subMenuEl.scrollHeight;
    }
    return menuHeight(subMenuItems);
  };
  ...
  useEffect(() => {
    ...
    arrayElement.forEach((dom) => {
      if (dom.parentElement.parentElement.parentElement.classList[0] === "nk-menu-item") {
        ...
        const subMenuHeight = getSubMenuHeight(dom.parentElement.parentElement, dom.parentNode.children);
  ...
  const menuToggle = (e) => {
    ...
    //For Sub Menu Height
    var subMenuHeight = getSubMenuHeight(subMenu, subMenuItem);
    // Get parent elements
...
const Menu = ({ sidebarToggle, mobileView, user }) => {
  const allRolesRaw = user?.all_roles || [];
  const all_roles = allRolesRaw.map((role) => (role || '').toString().toLowerCase());
  return (
    // Administrators and Super Administrators have full access
    all_roles.includes("super-administrator") || all_roles.includes("super-admin") || all_roles.includes("administrator") ?
      ...
      :
      // Managers have access up to Inventory
      all_roles.includes("manager") ? <ul className="nk-menu">
      ...
      all_roles.includes("customer-creator") ? <ul className="nk-menu">
        {menuCustomerCreator.map((item) =>
          item.heading ? (
            <MenuHeading heading={item.heading} key={item.heading} />
          ) : (
            <MenuItem
      ...
      all_roles.includes("engineer") ? <ul className="nk-menu">
        {menuEngineer.map((item) =>
          item.heading ? (
            <MenuHeading heading={item.heading} key={item.heading} />
          ) : (
            <MenuItem
      ...
      all_roles.includes("technician") ? <ul className="nk-menu">
        {menuTechnician.map((item) =>
          item.heading ? (
            <MenuHeading heading={item.heading} key={item.heading} />
          ) : (
            <MenuItem
      ...
      all_roles.includes("driver") ? <ul className="nk-menu">
        {menuDriver.map((item) =>
          item.heading ? (
            <MenuHeading heading={item.heading} key={item.heading} />
          ) : (
